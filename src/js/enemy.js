...
export class Enemy extends Actor {
  ...
  onInitialize(engine) {
    this.graphics.use(
      Resources.Snake.toSprite({destSize: {width: 32, height: 32}}),
    );

    // Patrol between min and max points
    if (this.minPoint && this.maxPoint) {
      this.actions.repeatForever((repeatCtx) => {
        repeatCtx.moveTo(this.minPoint, this.startY, this.speed);
        repeatCtx.moveTo(this.maxPoint, this.startY, this.speed);
      });
    }
